...
import tritonclient.http as client_util
...
# @torch.compile
def my_generate(model, tokenizer, inputs, max_new_tokens=100):
    out = []
    kv = None
    i = 0
    max_new_tokens = min(max_new_tokens, 2048-len(inputs))
    while i < max_new_tokens:
        res = model.forward(inputs.to(model.device), use_cache=True, past_key_values=kv)
        logit = res.logits #['logits']
        kv = res.past_key_values #['past_key_values']
        tok = torch.argmax(F.softmax(res.logits[0][-1]))
        out.append(tok)
        text = tokenizer.decode(out)
        if "\n" in text or tok == tokenizer.eos_token or i == max_new_tokens-1:
            return text
        i += 1
...
class CodeGenProxy:
    def __init__(self, host: str = 'localhost', port: int = 8000, verbose: bool = False):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            model, torch_dtype=torch.float32, low_cpu_mem_usage=True, trust_remote_code=True
        ).to("cuda")
        self.model = torch.compile(self.model)
        print("compiling")
        # my_generate(self.model, self.tokenizer, self.tokenizer.encode("foo", return_tensors="pt").to(self.model.device), max_new_tokens=2)
        self.model.generate(self.tokenizer.encode("foo", return_tensors="pt").to(self.model.device), max_new_tokens=1)
        print('compiled')
    ...
    def _newline_stop(self, inp, toks, scores):
        t = self.tokenizer.decode(toks[0][len(inp[0]):])
        return "\n" in t

    def generate(self, data):
        prompt = data['prompt']
        if 'suffix' in data:
            prompt = '<fim-prefix>' + prompt + '<fim-suffix>' + data['suffix'] + '<fim-middle>'
        n = data.get('n', 1)
        ...
        output_data = self.model.generate(
           input_ids.to(self.model.device), max_new_tokens=100, eos_token_id=self.tokenizer.eos_token_id,
           stopping_criteria=[partial(self._newline_stop, input_ids)]
        )
        out = output_data[0][len(input_ids[0]):]
        s = self.tokenizer.decode(out)
        s = s.split(self.tokenizer.eos_token)[0]
        # s = my_generate(self.model, self.tokenizer, input_ids, 100)
        print(s)
        print('-----------------------------')
        choices = []
        choice = {
...
#             max_tokens = self.MAX_MODEL_LEN - prompt_tokens - 5 # 5 is a buffer
...
#         sequence_lengths = len(output_data)
...
#             choices.append(choice)
        ...
        completion = {
            ...
            'usage': {
                ...
                'total_tokens': len(input_ids) + len(out),
            }
        }
